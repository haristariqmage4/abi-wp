...
{
    add_submenu_page('edit.php?post_type=mep_events', __('Attendee List', 'mep-form-builder'), __('Attendee List', 'mep-form-builder'), 'manage_options', 'attendee_list', 'mep_fb_attendee_list_dashboard');
}
...
?>
    ...
    <?php if($phone == 'on'){ ?> <td><?php echo get_post_meta($attendee_id, 'ea_tshirtsize', true) ?></td> <?php } ?>
    <?php if($tsize == 'on'){ ?> <td><?php echo get_post_meta($attendee_id, 'ea_phone', true) ?></td> <?php } ?>
...
function mep_attendee_query_stat($a_query, $event_id, $event_date)
{
    // echo $event_id;
...
?>
    <ul class="attendee_stat">
        ...
        <li class='showing_attendee'>
            ...
            echo $attendee_count;
            _e(' Attendee', 'mep-form-builder'); ?>
        </li>
        <li class='attendee_export_btn'>
            <?php if ($total_attendee_count > 0) { ?>

                <a class='mep_export_csv' href="edit.php?post_type=mep_events&page=attendee_list&action=download_csv_custom&event_id=<?php echo $event_id; ?>&ea_event_date=<?php echo $event_date; ?>" target='_blank'><span class="dashicons dashicons-media-text"></span> CSV Export Attendee</a>

            <?php } ?>
            <?php if ($event_id > 0) { ?>

                <a class='mep_export_csv' href="edit.php?post_type=mep_events&page=attendee_list&action=download_csv_extra_service&event_id=<?php echo $event_id; ?>&ea_event_date=<?php echo $event_date; ?>" target='_blank'><span class="dashicons dashicons-media-text"></span> CSV Export Extra Service</a>

            <?php } ?>
        </li>
         <?php do_action('mep_fb_attendee_list_head_btn_end',$a_query, $event_id, $event_date); ?>
    </ul>
<?php
}
    ...
    <div class="wrap">
        ...
        <div class='attendee_filter_section'>
            ...
            <li><?php _e('Filter List By:', 'mep-form-builder'); ?></li>
                <li><label for="event_filter"><input class='mep_attn_filter_by' type="radio" name='attendee_filter_by' id='event_filter' value='event' checked>Event</label></li>
                <li><label for="ticket_filter"><input class='mep_attn_filter_by' type="radio" name='attendee_filter_by' id='ticket_filter' value='ticket'>Ticket No</label></li>
                <li><label for="name_filter"><input class='mep_attn_filter_by' type="radio" name='attendee_filter_by' id='name_filter' value='name'>Name</label></li>
                <li><label for="order_filter"><input class='mep_attn_filter_by' type="radio" name='attendee_filter_by' id='order_filter' value='order'>Order</label></li>
                <li><label for="phone_filter"><input class='mep_attn_filter_by' type="radio" name='attendee_filter_by' id='phone_filter' value='phone'>Phone</label></li>
                <li><label for="email_filter"><input class='mep_attn_filter_by' type="radio" name='attendee_filter_by' id='email_filter' value='email'>Email</label></li>
            </ul>
            <ul>
...
function mep_fb_ajax_attendee_filter()
{
    ...
    $ev_filter_key          = $_REQUEST['ev_filter_key'];
    $checkin_status          = $_REQUEST['checkin_status'];
