...
add_action( 'init', 'mep_re_language_load');
function mep_re_language_load() {
	$plugin_dir = basename( dirname( __DIR__ ) ) . "/languages/";
	load_plugin_textdomain( 'mage-eventpress-re', false, $plugin_dir );
}
...
{
    $select_dateLabel=$select_dateLabel?:mep_get_option('mep_event_rec_select_event_date_text', 'label_setting_sec', __('Select Event Date:', 'mage-eventpress-re'));
    ob_start();    
                ...
                if ($mep_show_upcoming_event == 'yes') {
                    foreach ($event_multi_date as $event_date) {
                        ...
                        if (strtotime(current_time('Y-m-d H:i:s')) < strtotime(date('Y-m-d H:i:s', strtotime($start_date)))) {
                            ...
                            if ($cn == $cnt) {
                ?>
                                <input type='hidden' name="recurring_date" id="mep_recurring_date" value="<?php echo $start_date; ?>" />
                                <span class='mep-re-single-date' style='font-size:18px;font-weight: bold;'><?php echo get_mep_datetime($start_date, 'date-time'); ?></span>
                            <?php
                            }
                            $cn++;
                        }
                    ...
                    $cn = 1;
                    echo '    <select name="recurring_date" id="mep_recurring_date">';
                    ...
                    foreach ($event_multi_date as $event_date) {
                        ...
                        if (is_admin()) {
                            ...
                            if ($cn == $cnt) {
                            ?>
                                <option value="<?php echo $start_date; ?>"><?php echo get_mep_datetime($start_date, 'date-time'); ?></option>
                            <?php
                            }
                ...
                <table>
                    ...
                    if (strtotime($current_date) < strtotime($event_selected_date)) {
                        ...
                        foreach ($mep_event_ticket_type as $field) {
                            $qty_t_type             = isset($field['option_qty_t_type']) ? $field['option_qty_t_type'] : 'input';
                    ...
                    ?>
                        ...
                        <tr>
                            <td colspan=3>
                                <?php _e('Sorry, Event Date & Time is already over. Select another time.', 'mage-eventpress-rq'); ?>
                            </td>
                        </tr>
                    <?php
                    }
...
function mep_re_ajax_load_extra_service_list()
{
    ...
    $start_date = isset($_REQUEST['event_date']) ? $_REQUEST['event_date'] : '';
    $event_date = isset($_REQUEST['event_date']) ? $_REQUEST['event_date'] : '';
    $event_id   = isset($_REQUEST['event_id']) ? strip_tags($_POST['event_id']) : '';
    $post_id    = isset($_REQUEST['event_id']) ? strip_tags($_POST['event_id']) : '';
    $extra_service_label    = isset($_REQUEST['mep_extra_service_label']) ? strip_tags($_POST['mep_extra_service_label']) : 'Ex';
    $count                      = 1;
...
{
    $arr = mep_re_event_upcoming_date_filter($date, $event_id);
    return $arr;
}
...
function mep_re_get_everyday_event_date_sec($event_id)
{
    ...
    if (sizeof($global_on_days_arr) > 0) {
        $date_parameter= isset($_GET['date'] )?date('Y-m-d', $_GET['date']): null;
        ob_start();
        ?>
        <div class='mep_everyday_date_secs'>
            <ul>
                <li class='mep_re_datelist_label'>
                    <?php echo mep_get_option('mep_event_rec_select_event_date_text', 'label_setting_sec', __('Select Event Date:', 'mage-eventpress-re')); ?>
                </li>
                <li>
                    <?php
                    if (sizeof($global_on_days_arr) == 1) {
                    ?>

                        <span style='font-size: 20px;'><?php echo $date_parameter??$global_on_days_arr[0]; ?></span>
                        <input readonly type="hidden" name='mep_everyday_dates' id='mep_everyday_datepicker' value="<?php echo $date_parameter??$global_on_days_arr[0]; ?>">
                    <?php
                    } else {
...
function mep_re_default_load_ticket_time_list($event_id, $event_date)
{
    ...
    if ((is_array($event_off_days) && sizeof($event_off_days) > 0 && in_array($selected_day, $event_off_days)) || in_array($event_date, $global_off_dates_arr)) {
        ...
        echo mep_get_option('mep_event_rec_day_off_text', 'label_setting_sec', __('Day Off', 'mage-eventpress-re'));
    } else {
    ?>
...
function mep_re_ajax_load_ticket_time_list()
{
    if (wp_verify_nonce( $_POST['nonce'], 'mep-ajax-recurring-nonce' ) ) {
        ...
        if ((is_array($event_off_days) && sizeof($event_off_days) > 0 && in_array($selected_day, $event_off_days)) || in_array($event_date, $global_off_dates_arr)) {
            ...
            <input type="hidden" name='time_slot_name' id='time_slot_name' value=''>
            <select name="ea_event_date" id="mep_everyday_ticket_time">
                <option value="0"><?php echo mep_get_option('mep_event_rec_select_a_time_text', 'label_setting_sec', __('Please Select A Time', 'mage-eventpress-re')); ?></option>
                <?php apply_filters('mep_everyday_time_list_item', mep_get_everyday_time_list($event_id, $event_date), $event_id); ?>
            </select>
        <?php
...
function mep_re_display_cart_data($cart_item)
{
    ...
    if ($time_slot) {
    ?>
        <li><?php echo mep_get_option('mep_event_rec_time_slot_text', 'label_setting_sec', __('Time Slot:', 'mage-eventpress-re'));
            echo $time_slot; ?></li>
    <?php
    }
}
...
function mep_re_add_cart_order_data($values, $item)
{
    ...
    if ($cart_location) {
        $item->add_meta_data(mep_get_option('mep_event_rec_time_slot_text', 'label_setting_sec', __('Time Slot:', 'mage-eventpress-re')), $cart_location);
...
function mep_re_show_data_in_pdf($ticket_id, $event_id = '', $order_id = '', $ticket_type = '')
{
    ...
    if ($time_slot) {
    ?>
        <li><strong><?php echo mep_get_option('mep_event_rec_time_slot_text', 'label_setting_sec', __('Time Slot:', 'mage-eventpress-re')); ?></strong> <?php echo $time_slot;  ?></li>
        <?php
    }
}
...
add_action('wp_footer', 'mep_re_script', 99);
add_action('mep_after_event_cart_shortcode', 'mep_re_script', 99);
function mep_re_script($eid = '')
{
    require_once(dirname(__DIR__) . "/js/datepicker_calculation.php");
}
...
function mep_re_style_strings_reg($default_translation)
{
    $recurring_translation = array(
        array(
            ...
            'label' => __('Recurring/Repeated Datepicker Background Color', 'mage-eventpress-re'),
            'desc' => __('Select a color for Recurring/Repeated Datepickers Background', 'mage-eventpress-re'),
            'default' => '#ffbe30',
            'type' => 'color',
        ),
        array(
...
if($recurring == 'everyday'){
    $event_state = __('Recurring Event (Repeated)','mage-eventpress-recurring');
}elseif($recurring == 'yes'){
    $event_state = __('Recurring Event (Selected Dates)','mage-eventpress-recurring');
}else{
    $event_state = '';
}
